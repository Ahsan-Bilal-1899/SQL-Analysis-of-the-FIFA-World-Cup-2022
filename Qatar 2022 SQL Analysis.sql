...
where G.Round != 'FINAL'
and T.Team_Name in ('Argentina, France') 
group by T.Team_Name, G.Round
...
/* What was the highest attendance recorded in each round? */

select distinct S.Round, S.Stadium_Name, MAX(S.Attendance) as Highest_Attendance
from Stadiums S
group by S.Round, S.Stadium_Name,
order by Highest_Attendance desc
